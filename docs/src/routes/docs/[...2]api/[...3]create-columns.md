...
#### `columnDef.cell?: (bodyCell, state) => RenderConfig`

Defines the component to use for the body cells of the data column.

`cell` is a function that receives [`BodyCell`](./body-cell.md) and [`TableState`](./table-state.md), and returns a [`RenderConfig`](./--render.md#renderconfig).

_Defaults to returning `bodyCell.value`_.
